...
var search_url = 'https://%s' + search_path + '?q=%s&start=%s&gbv=1&nfpr=1'
	...
	request(options, function(error, response, body){
		if (!error && response.statusCode == 200) {
			...
			$('div.g').each(function(i, element){
				...
				var content_html = $(this).find('.st').html() || '';

                if (stringStartsWith(url, "/url?")) {
                    var parsed_url = parse_url(url, google_hostname) || '';
                    if (stringStartsWith(parsed_url, 'http://') || stringStartsWith(parsed_url, 'https://')) {
                        results.push({ title: title, title_html: title_html, url: parsed_url, content: content, content_html: striptags(content_html, ['b']) });
                    }
                } else if (stringStartsWith(url, "/search?")) {
                    if (isImagesUrl(url)) {
                        results.push({ imagespreview: true });
                    } else if (isNewsUrl(url)) {
                        results.push({ newspreview: true });
                    }
                }
			});
			
			$('p._Bmc').each(function(i, element) {
				results.push({ suggestion: $(this).text() });
			});
			
			var didyoumean = $('#_FQd a').text() || "";
			if (didyoumean != "") {
			    results.push({ didyoumean: didyoumean });
			}
...
function isImagesUrl(url_string) {
    try {
        if (url_string == '') {
            return false;
        } else {
            var parsed_url = url.parse(url_string, true);
            if (parsed_url.query.tbm == "isch") {
                return true;
            } else {
                return false;
            }
...
function isNewsUrl(url_string) {
    try {
        if (url_string == '') {
            return false;
        } else {
            var parsed_url = url.parse(url_string, true);
            if (parsed_url.query.tbm == "nws") {
                return true;
            } else {
                return false;
            }
